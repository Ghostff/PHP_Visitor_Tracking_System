...
class Stalk
{
    ...
    public $dma_code       = null; # metro and dma code are the same. use metro_code
    ...
    private function commonGetRecord($gi, $seek_country)
    {
        ...
        $this->country_code = $gi->GEOIP_COUNTRY_CODES[$char];
        $this->country_code3 = $gi->GEOIP_COUNTRY_CODES3[$char];
        $this->country_name = $gi->GEOIP_COUNTRY_NAMES[$char];
        $this->continent_code = $gi->GEOIP_CONTINENT_CODES[$char];
        $record_buf_pos++;
        $str_length = 0;
        ...
        {
            // var_dump($gi);
            $this->region = substr($record_buf, $record_buf_pos, $str_length);
        }
        ...
        if ($str_length > 0)
        {
            $this->city = substr($record_buf, $record_buf_pos, $str_length);
        }
        $record_buf_pos += $str_length + 1;
        $str_length = 0;
        ...
        if ($str_length > 0)
        {
            $this->postal_code = substr($record_buf, $record_buf_pos, $str_length);
        }
        $record_buf_pos += $str_length + 1;
        ...
        $this->latitude = ($latitude / 10000) - 180;
        for ($j = 0; $j < 3; ++$j)
        {
            $char = ord(substr($record_buf, $record_buf_pos++, 1));
            $longitude += ($char << ($j * 8));
        }
        $this->longitude = ($longitude / 10000) - 180;
        if (GEOIP_CITY_EDITION_REV1 == $gi->databaseType)
        {
            ...
            if ($this->country_code == 'US')
            {
                ...
                $this->metro_code = $this->dma_code = floor($metroarea_combo / 1000);
                $this->area_code = $metroarea_combo % 1000;
            }
        }
    ...
    private function getRecord($gi)
    {
        if (! $this->ip)
        {
            throw new RuntimeException('No IP set');
        }

        $seek_country = _geoip_seek_country($gi, ip2long($this->ip));
        if ($seek_country == $gi->databaseSegments)
        {
            return null;
        }

        return $this->commonGetRecord($gi, $seek_country);
    }
